...
import { scanDriverUpdates, installDriverUpdates } from './driver-manager.ipc'
...
export function registerDriverAgentIpc(_getWindow: WindowGetter): void {
  ipcMain.handle(IPC.DRIVER_AGENT_EVALUATE, async (): Promise<AgentEvaluationResult> => {
    ...
    try {
      ...
      getLogger().success('driver-agent', `Evaluation complete: ${result.criticalCount} critical, ${result.recommendedCount} recommended, ${result.optionalCount} optional, ${result.cautionCount} caution, ${result.skipCount} skip`)
  ...
  ipcMain.handle(IPC.DRIVER_AGENT_APPROVE, async (_event, request: unknown): Promise<{ success: boolean; error?: string; rebootRequired?: boolean }> => {
    ...
    try {
      ...
      getLogger().success('driver-agent', `Installed ${result.installed}, failed ${result.failed}, reboot: ${result.rebootRequired}`)
      return {
        success: result.failed === 0,
        error: result.errors.length > 0 ? result.errors.map((e) => `${e.deviceName}: ${e.reason}`).join('; ') : undefined,
