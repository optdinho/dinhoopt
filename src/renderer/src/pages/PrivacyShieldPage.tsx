...
export function PrivacyShieldPage({ embedded }: { embedded?: boolean }) {
  ...
  useProgressListener(
    window.dinho.onPrivacyProgress,
    (data) => usePrivacyStore.getState().setProgress(data)
  )
                    ...
                    {catSettings.map((setting, i) => {
                      ...
                      <div key={setting.id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="mt-0.5 text-[11px]" style={{ color: 'var(--text-dim)' }}>{t(`privacy.descriptions.${setting.id}`, setting.description)}</p>
