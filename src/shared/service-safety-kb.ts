import type { ServiceCategory, ServiceSafety } from './types'

export interface ServiceSafetyEntry {
  safety: ServiceSafety
  category: ServiceCategory
  note: string
  /** Games that may be negatively affected if this service is disabled */
...
export const SERVICE_SAFETY_KB: Record<string, ServiceSafetyEntry> = {
  ...
  adpsvc: {
    safety: 'caution',
    category: 'telemetry',
    note: 'Aggregator Device Platform — telemetry and data aggregation',
    incompatibleGames: ['fivem', 'minecraft'],
  },
  cdpsvc: {
    safety: 'caution',
  ...
  ssdpsrv: {
    safety: 'caution',
    category: 'network',
    note: 'SSDP Discovery — needed for network device discovery and LAN multiplayer',
    incompatibleGames: ['fivem', 'minecraft'],
  },
  winmgmt: {
    safety: 'unsafe',
 ...
 */
/**
 * Check if a service is compatible with a given game.
 * Returns `true` if the service has no incompatibility with the game.
 */
export function isGameCompatible(serviceName: string, gameName: string): boolean {
  const entry = lookupServiceSafety(serviceName)
  if (!entry.incompatibleGames || entry.incompatibleGames.length === 0) return true
  const game = gameName.toLowerCase()
  return !entry.incompatibleGames.some((g) => game.includes(g))
}
